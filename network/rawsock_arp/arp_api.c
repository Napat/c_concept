...
static int convert_ipstr_to_array(uint8_t* source_ip, uint8_t* ip_array_out){
    uint8_t index;
...
int32_t arp_request(char* intf_name, uint8_t* target_ip, uint8_t* source_ip){
    ...
    convert_ipstr_to_array(source_ip, src_ip);
    convert_ipstr_to_array(target_ip, tar_ip);
    for(index=0; index<IPV4_LENGTH; index++){
